...
\author{\IEEEauthorblockN{José Raimundo Barbosa\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}\IEEEauthorrefmark{3}, 
Jean Felipe Fonseca de Oliveira\IEEEauthorrefmark{5}
Carlos Danilo Miranda Regis\IEEEauthorrefmark{1}\IEEEauthorrefmark{2},
Ruan Delgado Gomes\IEEEauthorrefmark{1}\IEEEauthorrefmark{3}\\}
\IEEEauthorblockA{\IEEEauthorrefmark{1}Instituto Federal de Educação, Ciência e Tecnologia da Paraíba\\}
\IEEEauthorblockA{\IEEEauthorrefmark{2}Grupo de Processamento de Sinais Digitais\\}
\IEEEauthorblockA{\IEEEauthorrefmark{3}Laboratório de Computação Embarcada e Distribuída\\}
\IEEEauthorblockA{\IEEEauthorrefmark{4}TPV Technology, São Paulo, Brasil}
...
To promote HEVC optimization research, JCT-VC provided the HM reference encoder which features a  \textcolor{red}{basic} implementation of HEVC that makes possible  to carry out studies on the efficiency of the codec and develop new features and optimizations~\cite{itu:10}. However, evaluating codecs developed based on the HM reference source code is a normally time-consuming process, since each test implies in the execution of several instances of the evaluated codec and the reference codec, which can last for several hours. After execution, is necessary to compare the UHD videos using objective or subjective metrics. With the comparisons it is possible to monitor the efficiency and performance of the new encoders compared to stable versions in the UHD scenario~\cite{netflix:16}.
